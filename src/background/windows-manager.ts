...
// Default screen dimensions when screen API is not available (service worker)
...
async function getScreenDimensions(): Promise<{
  ...
  // In MV3 service worker, we don't have access to window.screen
  // Try to get dimensions from a display info API if available
  try {
...
export class MainWindowsManager {
  ...
  async makeRoomForSidebar(
    ...
    const sidebarWidth =
      (sidebarSnapshot && sidebarSnapshot.width) || appConfig?.panelWidth || 450
...
export class QsPanelManager {
  ...
  async getDefaultRect(): Promise<WinRect> {
    const appConfig = getAppConfig()
    const qsLocation = appConfig?.qsLocation || 'CENTER'
    const qssaHeight = appConfig?.qssaHeight || 400
    const panelWidth = appConfig?.panelWidth || 450
    ...
    switch (qsLocation) {
      case 'CENTER':
        qsPanelLeft = (screen.width - qsPanelWidth) / 2
        qsPanelTop = (screen.height - qssaHeight) / 2
        break
      case 'TOP':
        qsPanelLeft = (screen.width - qsPanelWidth) / 2
        qsPanelTop = 30
        break
      case 'RIGHT':
        qsPanelLeft = screen.width - qsPanelWidth - 30
        qsPanelTop = (screen.height - qssaHeight) / 2
        break
      case 'BOTTOM':
        qsPanelLeft = (screen.width - qsPanelWidth) / 2
        qsPanelTop = screen.height - qsPanelHeight - 10
        break
      case 'LEFT':
        qsPanelLeft = 10
        qsPanelTop = (screen.height - qssaHeight) / 2
    ...
    return {
      top: Math.round(qsPanelTop + screen.availTop),
      left: Math.round(qsPanelLeft + screen.availLeft),
  ...
  async getSidebarRect(side: 'left' | 'right'): Promise<WinRect> {
    const appConfig = getAppConfig()
    const panelWidth =
      (this.snapshot && this.snapshot.width) || appConfig?.panelWidth || 450
    ...
    return mainWin &&
      ...
      : {
          top: 0,
          left: Math.round(
            side === 'right' ? screen.width - panelWidth : 0
          ),
          width: Math.round(panelWidth),
